...
//var draculaInit = function (canvas) {
    var canvas = "canvas";
	if (! $("#" + canvas).length)
	{
		console.error("no canvas for dracula");
	}
    var width = 400; //$("#" + canvas).width() - 20;
    var height = 600; //$("#" + canvas).height() - 60;
...
//        render : render
    ...
    var draculaRedraw = function(canvas) {
        layouter.layout();
        renderer.r.setViewBox(0, 0, 350, 60);
        renderer.r.setSize($("#" + canvas).width(), $("#" + canvas).height());
        renderer.draw();
    };
    var draculaHide = function(id) {
        g.nodes[id].hide();
    };
    var draculaShow = function(id) {
        g.nodes[id].show();
    };
