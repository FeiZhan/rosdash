		...
		$.widget("mn.sDashboard", {
			...
			_bindEvents : function() {
				...
				this.element.on("click",".sDashboardWidgetHeader span.ui-icon.ui-icon-circle-plus",function(e){
					...
					} else if (widgetDefinition.widgetType === "gmap")
					{
						ROSDASH.resizeGmap();
					} else if (widgetDefinition.widgetType === "arbor")
					{
						//arborInit();
					} else if (widgetDefinition.widgetType === "network")
					{
			...
			_constructWidget : function(widgetDefinition) {
				...
				if (widgetDefinition.widgetType === 'table') {
					...
					widgetContent = $('<div id="map-canvas" class="sDashboardWidgetContent" />');
				} else if (widgetDefinition.widgetType === 'arbor') {
					widgetContent = $('<canvas id="viewport" class="ArborWidgetContent"></canvas>');
				} else if (widgetDefinition.widgetType === 'network') {
					widgetContent = $('<div id="dracula_canvas" class="draculaWidgetContent"></div>');
				} else {
			...
			addWidget : function(widgetDefinition) {
				...
				if (widgetDefinition.widgetType === "gmap")
				{
					ROSDASH.initGmap();
				} else if (widgetDefinition.widgetType === "arbor")
				{
			...
			//@fzhan set the wiget data
			setContentById : function(id, content) {
				var all_data = this.options.dashboardData;
				var target;
				for (var i = 0; i < all_data.length; i ++)
				{
					var widgetObject = all_data[i];
					if (widgetObject.widgetId === id)
					{
						this.options.dashboardData[i].widgetContent = content;
						var new_widget = this.options.dashboardData[i];
						var widget = this.element.find("li" + "#" + all_data[i].widgetId);
						if (widget.length > 0)
						{
							var a = widget.find("div").find("div:nth-child(2)").html(content);
						}
						break;
					}
				}
